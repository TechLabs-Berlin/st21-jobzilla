...
class Logistics_form_helper
{
     ...
     * @return string
     */
    static function render_field($xmlObject, $selected = '', $permissions, $id = 0, $form_orientation = "", $field_cols = "", $label_cols = "")
    {
        $CI = & get_instance();
        ...
        $checkbox_tooltip = filter_var($xmlObject['checkbox_tooltip'], FILTER_VALIDATE_BOOLEAN);
        $single_checkbox_tooltip = filter_var($xmlObject['single_checkbox_tooltip'], FILTER_VALIDATE_BOOLEAN);
        ...
        $field_status = "";
        $lable_column = 0;
        $lable_class = (string) $xmlObject['label_class'];
        $ext_div_start = "";
        $info_btn = "";
        ...
        if ($type != 'hidden') {
            ...
            if (! empty($form_orientation) && $form_orientation == "horizontal") {
                ...
                if ( strlen($label_cols) > 0 ) {
                    $lable_cols = $label_cols;
                }else {
                    $lable_cols = 12 - $field_cols;
                }
                
                $lable_class =  $lable_class." col-$lable_cols";
        ...
        if( $checkbox_tooltip == true){
            $function_name = 'render_custom_' . strtolower($type);
        }

        if( $single_checkbox_tooltip == true){
            $function_name = 'render_customs_' . strtolower($type);
        }
        /*
         ...
         * render_{type_name}
         */
        if (method_exists('Logistics_form_helper', $function_name)) {
     ...
     * render input filed
     */
    static function render_custom_input($xmlObject, $selected, $extra, $field_status)
    {
        $name = $xmlObject['name'];
        $placeholder = (string) $xmlObject['placeholder'];
        if (isset($placeholder[0]) and $placeholder[0] == '_') {
        ...
        $CI = &get_instance();
        $prefix = $CI->config->item($CI->config->item("settingsPrefix")."_NO_PREFIX");

        $numberWithPrefix = explode(',', $selected);
        $prefixShow = !empty($numberWithPrefix[0]) ? $numberWithPrefix[0] : $prefix;
        $number = !empty($numberWithPrefix[1]) ? $numberWithPrefix[1] : '';

        $output_html .= '<div class="input-group"><span class="input-group-addon">'.$prefixShow.'</span>';
        $output_html .= form_input($data, $number, $field_status);
        $output_html .= '</div>';
    ...
    public static function render_info_button($name, $type, $hide_tooltip, $extra_style = '', $form_orientation='')
    {
        ...
        if ($type == "hidden" || strlen($description) < 1) {
            ...
            $html .= '<a style="color:white;" class="m-btn m-btn--icon m-btn--icon-only m-btn--pill m--pull-left custom-info-button" data-trigger="click" data-html="true" data-animation="true" data-toggle="popover" data-content="' . htmlspecialchars($description, ENT_QUOTES) . '">';
    ...
    static function render_money($xmlObject, $selected, $extra, $field_status)
    {
        ...
        $show_percentage = (integer) $xmlObject['show_percentage_symbol'];
        $percentage_show = (string) $xmlObject['percentage_show'];
        $default_currency = $xmlObject['symbol'];
        
        if ($show_currency > 0 || $show_percentage > 0) {
        ...
        if ( strlen($selected) > 0) {
            if(strlen($fractionDigits) > 0){
				...
				if ( $thousandSeperator == '1' ) {
					$selected_value = self::customeMoneyFormate(number_format($selected, $fractionDigits));
				}else {
        ...
        if ($show_currency > 0) {
            ...
            $output_html .= '<span class="input-group-addon currency-symbol">' . $currency_show . '</span>';
            $output_html .= form_input($data, $selected_value, $field_status, $align);
            $output_html .= '</div>';
        } else if ($show_percentage > 0) {

            $output_html .= form_input($data, $selected_value, $field_status, $align);
            $output_html .= '<span class="input-group-addon percentage-symbol">' . $percentage_show . '</span>';
            $output_html .= '</div>';
        } else {
            $output_html .= form_input($data, $selected_value, $field_status, $align);
        }
    ...
    static function render_single_checkbox($xmlObject, $selected, $extra, $field_status)
    {
        ...
        $output_html .= "<label class='m-checkbox'><input type='checkbox' name='" . $name . "' class='" . $extra . "' id='" . $name . "' value='" . '1' . "' " . $checked . ' ' . $field_status . ">" . $checkboxText . "<span > </span></label>";
        $output_html .= "</div>";
    ...
    static function render_lookup($xmlObject, $selected, $extra, $field_status)
    {
        ...
        } else if($name == "show_branch_info_in_deliveries"){
            $options["0"] = dashboard_lang('_NO');
        } else {
        ...
        $concatMultiple = intval($xmlObject['concat_multiple']);
        ...
        if (isset($autosuggest) && $autosuggest == 1) {
            ...
            if ( $concatMultiple == '1' ) {
                $query = $CI->dashboard_model->lookup_with_concat_multuple($tableName, $key, $valueArray, $orderBy, $orderOn, $additional_and, $account_id, $selected);
            }else {
            ...
            if (isset($custom_function) and $custom_function == 'getAllPermittedRoles') {
                ...
                if ( $concatMultiple == '1' ) {
                    ...
                    $query = $CI->Dashboard_Logistics_Model->non_ajax_lookup_with_seperator($tableName, $key, $valueArray, $orderBy, $orderOn, $additional_and, $account_id);
                
                }else {
                    $query = $CI->Dashboard_Logistics_Model->non_ajax_lookup($tableName, $key, $valueArray, $orderBy, $orderOn, $additional_and, $account_id);
                }

                foreach ($query->result() as $row) {
                    if ($xmlObject['is_translated'] == 1) {

                        $langKey = strtoupper($row->{"select_value"});
                        ...
                        $options[$row->{"select_key"}] = dashboard_lang($langKey);
     ...
     * renders Lookup
     */
    static function render_uom_lookup($xmlObject, $selected, $extra, $field_status)
    {
        $options = array();
        $output_html = '';
        $lookup_default_value = (string) $xmlObject['default_value'];
        ...
        if (isset($autosuggest) && $autosuggest == 1) {
            ...
            if (isset($custom_function) and $custom_function == 'getAllPermittedRoles') {
                ...
                $query = $CI->Items_Model->getUomByItems ( $xmlObject['items_id'], $account_id);
    ...
    static function render_image($xmlObject, $selected, $extra, $field_status)
    {
        ...
        $output_html .= boo2_render_image($selected, $img_upload_path, $sub_directory, $name, $field_status, $ajaxSavingClass);
        $data = array(
            'name' => $name,
            'id' => $name,
            'class' => $extra
    ...
    static function render_file($xmlObject, $selected, $extra, $field_status)
    {
        ...
        $ajaxSavingClass="";
        if ( stripos($extra, "ajax-save", 0) !== false ) $ajaxSavingClass = "ajax-save";
        $output_html .= boo2_render_file($selected, $img_upload_path, $name, $field_status, $ajaxSavingClass);
        $data = array(
            'name' => $name,
            'id' => $name,
            'class' => $extra
     ...
     * render checkbox
     */
    static function render_custom_single_checkbox($xmlObject, $selected, $extra, $field_status)
    {
        $output_html = '';
        $name = (string) $xmlObject['name'];
        $checkboxText = dashboard_lang("_" . strtoupper($name) . "_TEXT");
        ...
        $output_html .= self::render_info_button($name, '', false, 'margin-bottom: 11px;margin-left: -10px;', "horizontal");
     ...
     * render checkbox
     */
    static function render_customs_single_checkbox($xmlObject, $selected, $extra, $field_status)
    {
        $output_html = '';
       
        $name = (string) $xmlObject['name'];
        $checkboxText = dashboard_lang("_" . strtoupper($name) . "_TEXT");
        $checkboxName = (string) $xmlObject['prepend_table_name'].'_'.(string) $xmlObject['name'].'_TEXT';
        $checkboxName = strtoupper($checkboxName);
        ...
        $output_html .= self::render_info_button($checkboxName, '', false, 'margin-bottom: 4px;margin-left: -10px;', "horizontal");

        $output_html .= "</div>";
        return $output_html;
    }

    public static function render_info_button_pull_left($name, $type, $hide_tooltip, $extra_style = '', $form_orientation='')
    {
        $description_key = "_" . strtoupper($name) . "_INFO";
        $description = dashboard_lang($description_key);
        $extraClassName = "";
        ...
        if ($type == "hidden" || strlen($description) < 1) {
            ...
            $html = '<span style="' . $extra_style . '" class="'.$extraClassName.' m--padding-left-10 m--valign-middle m--pull-left"> ';
            $html .= '<a style="color:white;" class="m-btn m-btn--icon m-btn--icon-only m-btn--pill m--pull-left custom-info-button" data-trigger="click" data-html="true" data-animation="true" data-toggle="popover" data-content="' . htmlspecialchars($description, ENT_QUOTES) . '">';
            $html .= '<i class="fa fa-info"></i></a>';
            $html .= '</span>';
        }
