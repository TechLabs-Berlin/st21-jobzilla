...
class Items_form_helper
{
    ...
    static function render_field($xmlObject, $selected = '', $permissions, $id = 0, $form_orientation = "", $field_cols = "", $label_cols = "")
    {
         ...
         * render_{type_name}
         */
        if (method_exists('Items_form_helper', $function_name)) {
    ...
    static function render_image($xmlObject, $selected, $extra, $field_status)
    {
        ...
        $output_html .= items_render_image($selected, $img_upload_path, $sub_directory, $name, $field_status, $ajaxSavingClass);
        $data = array(
            'name' => $name,
            'id' => $name,
            'class' => $extra
    ...
    static function render_file($xmlObject, $selected, $extra, $field_status)
    {
        ...
        $ajaxSavingClass="";
        if ( stripos($extra, "ajax-save", 0) !== false ) $ajaxSavingClass = "ajax-save";
        $output_html .= items_render_file($selected, $img_upload_path, $name, $field_status, $ajaxSavingClass);
        $data = array(
            'name' => $name,
            'id' => $name,
            'class' => $extra
 ...
 */
function items_render_image($src_image, $img_upload_path, $sub_directory, $name, $field_status, $ajaxSavingClass="", $width=100 , $height=0, $noCheckboxDelete=0)
{
    ...
    if ((stripos($src_image, "http://") !== false) || (stripos($src_image, "https://") !== false)) {
        ...
        if ( $noCheckboxDelete == 0 ) {
            $html .= '<a href="javascript:void(0);" data-id="50" class="delete-checkbox ajax-save-data delete-item-image"><i class="fa fa-remove"></i></a>';
        }
        $html .= '</div>';
    } else {
