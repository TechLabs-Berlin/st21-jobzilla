...
class Amazon_s3_model extends CI_Model
{
    public function __construct()
    {
        parent::__construct();
        $this->load->config("amazon_s3");
        $this->load->library("portal/s3");
        $this->load->library("portal/Aws3");
    }  
    ...
    public function uploadFileToS3($tableName, $fileName, $fieldName)
    {
        ...
        $s3Url = $this->config->item("s3_url");
        $bucket = $this->config->item("bucket");
        $directory = $this->config->item("s3_directory");
        if(!empty($directory)){
    ...
    public function deleteFileFromS3($tableName, $fieldName, $subDirectory="", $id = 0)
    {
        $returArray = array(
            "status" => false
        );
        ...
        $this->db->where('id', $id);
        $dataObj = $this->db->get($tableName)->row();
